...
def predict_x0_from_eps(x_t, t, eps, alphas_cumprod):
    ...
    ac_batch = extract_into_batch(alphas_cumprod, t, x_t)

    x0_hat = (
        x_t - torch.sqrt(1.0 - ac_batch) * eps
    ) / torch.sqrt(ac_batch)
...
def ddpm_experiment(n_data: int = 64, size: int = 8, T: int = 20, hidden: int = 16, num_steps: int = 40, batch_size: int = 16, lr: float = 5e-2, n_samples: int = 8, seed: int = 0) -> dict:
    ...
    # 1. construct dataset
    dataset = make_blob_dataset(n=n_data, size=size, seed=seed)

    # 2. init denoising params
    schedule = build_diffusion_schedule(T)
    params = init_tiny_unet(hidden=hidden, time_dim=hidden, seed=seed)
    
    # 3. start training 
    trained_params, history = train_ddpm(dataset, params, schedule, num_steps, batch_size, lr, seed)
    
    # 4. sampling
    """
    (遗漏) 不知道正确的batches of image samples形状
    """
    # ****************
    sample_shape = (n_samples, 1, size, size)
    # ****************
    gen_samples = ddpm_sample_loop(params=params, schedule=schedule, shape=sample_shape, seed=seed)
    
    # 5. evaluation (生成纯高斯噪声作为 baseline)
    torch.manual_seed(seed + 2)
    sample_mse = sample_quality_mse(gen_samples, dataset)

    noise_samples = torch.randn(sample_shape)
    noise_mse = sample_quality_mse(noise_samples, dataset)
    improvement = noise_mse - sample_mse
    # 错误理解
    # noise_samples = torch.randn_like(gen_samples)
    # noise_mse = noise_prediction_loss(gen_samples, noise_samples)

    # final loss
    final_loss = history[-1] if len(history) > 0 else float("nan")

    # Note: 提示里要求返回dict，注意类型转换
    return {
